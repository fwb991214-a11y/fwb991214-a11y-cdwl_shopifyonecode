...
// subcollection-load-more (custom sub-collections on collection page)
var SubcollectionLoadMore = class extends HTMLElement {
  constructor() {
    super();
    this._onClick = this._onClick.bind(this);
  }
  connectedCallback() {
    this._step = parseInt(this.dataset.step || "6", 10);
    this._itemsContainer = this.querySelector("[data-items]");
    this._button = this.querySelector("[data-load-more]");
    this._showMore = this.querySelector("[data-show-more]");
    this._loading = this.querySelector("[data-loading]");
    if (!this._itemsContainer || !this._button) {
      return;
    }
  ...
  _hiddenItems() {
    return Array.from(this._itemsContainer.querySelectorAll("[data-hidden-item]"));
  }
  _setLoading(isLoading) {
    if (!this._showMore || !this._loading) return;
    this._showMore.hidden = isLoading;
    this._loading.hidden = !isLoading;
    this._button.toggleAttribute("aria-busy", isLoading);
    this._button.disabled = isLoading;
  }
  _syncButtonVisibility() {
    const hasMore = this._hiddenItems().length > 0;
    this._button.hidden = !hasMore;
  }
  _revealNextBatch() {
    const hidden = this._hiddenItems();
    hidden.slice(0, this._step).forEach((el) => {
      el.hidden = false;
      el.removeAttribute("data-hidden-item");
    });
  }
  _onClick() {
...
if (!window.customElements.get("subcollection-load-more")) {
  window.customElements.define("subcollection-load-more", SubcollectionLoadMore);
}
export {
  AccordionDisclosure,
  AccountLogin,
